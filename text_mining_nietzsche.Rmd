...
To begin I create a character vector of partial urls for each of Nietzsche's books. They have been made available by the [Gutenberg project](http://www.gutenberg.org/), which is an online resource for free e-books.
```{r}
...
In this section of code an empty character vector is first defined that will hold the full urls for each book. The `make_urls` function creates the full url from the partial urls in `nietz_url`. The full urls are then then saved into `nietz_book_urls`.
...
Now the `dplyr` package is imported to use the `data_frame` function, as well as for other features like piping (%>%), etc.

Each line of text for each book is contained in `full_texts` within a single column, along with a second column that contains the book title. The function `read_books` scrapes each text from the Gutenberg site, which is provided in .csv format. In some of the books the text for a given line was spread across 2 columns, instead of 1. The function corrects for this by gathering these columns into a single column through the if/else statement. Finally, the list is converted into a data frame and saved into `full_texts`. The `read_books` function is then called, and the output is saved into `nietz_full_texts`. 
...
Stop words are highly common words, such as *the* and *if*, that are in many ways uninformative for topic modeling and need to be removed. This is done below with a list called `stop_words` provided by the `tidytext` library. 

I also define a list of custom stop words for archaic English that is used in some of the translations of Nietzsche's books. After that any remaining underscores that are still present due to abnormal punctuation and spacing are removed. 
...
Now it is time to *tokenize* the words (i.e. make one word per row) with `unnest_tokens` while also filtering out stop words. The `group_by` function (from dplyr) allows the grouping of variables, such that operations are performed separately for given levels within a variable. The `mutate` function (also from dplyr) is then used to create a new variable (i.e. new column) that contains the row number for each line of text. The text is then ungrouped, and the `unnest_tokens` function (from tidytext) places each word into a single row. And then we ensure that the supplementary texts are excluded, as well as the stop words from `stop_words` and `custom_stop_words`. The data is now in tidy format. 
...
On to the word cloud! To begin, the `wordcloud` library is loaded and the background and color scheme of the word cloud are defined. The word frequency is tallied with the function `count` (from dplyr).  The top 100 most frequent words are included in the plot. It reflects the most utilized words, apart from stop words, throughout all of his books, and is a marker for the topics Nietzsche discusses in general. 

```{r, fig.width=5, fig.height=5, fig.align="left", message=FALSE}
library(wordcloud)
par(bg="white")
...
Here the *term-frequency-inverse-document-frequency* (tf-idf) is calculated for each word. This is a way of analyzing a term's uniqueness within each individual book. 

First a list `nietz_book_words` is created that contains all of Nietzsche's words counted separately for each book. Another variable `nietz_total_words` is used to count the total number of words used within each book. The 'total words' column from `nietz_total_words` is then attached to `nietz_book_words` via `left_join`. 
...
The tf-idf is calculated separately for each word within each book using `bind_tf_idf` from the `tidytext` library. After that the data are rearranged in `nietz_book_words`, such that the words with the highest tf-idf appear at the top of the column. The 'word' column is also converted into a factor, with the levels being equal to the reverse number of times a word appears (see `rev`). After the 6 books that I am interested in are selected, with each book being filtered for the 15 words with the highest tf-idf.
...
Before plotting the data saved in `plot_nietz_bar`, `ggplot2` is loaded for the upcoming visual graphics. The `fill` option makes the bars a different color for each book, and `geon_bar` tells `ggplot` to make a bar graph (without a legend). The x and y variables are then labeled and `facet_wrap` organizes the 6 plots into a single graphic. The x and y coordinates are then flipped so that the bars appear horizontal.  

```{r, echo=FALSE, fig.width=11, fig.height=6, fig.align="center", message=FALSE}
library(ggplot2)
## GRAPH 3 ##
plot_nietz_bar %>% 
...
Next the lists `wephil` and `seaout` are created to hold text from the books *We Philologists* and *Thoughts Out of Season: Part 2*, respectively.
...
A function `make_book_sections` is then defined, which divides each book into 10 separate (but equally sized) sections. The column `section` is also created to indicate the 'section' number for each line of text. 
...
The `tidyr` library is loaded in order to use the `unite` function. The `make_book_sections` function divides each book into 10 sections. The `section` and `title` variables are also concatenated into a single variable called `document`. 
...
Here the text is tokenized and tidied, and then the stop words are removed. The words are then counted and sorted from highest to lowest.  
...
The *Latent Dirichlet Allocation* (LDA) model is run with `LDA` that comes from the library `topicmodels`. Ten (k) topics are specified for each book, and is seeded with 1234, rather than a random number, for reproducibility purposes. 
...
The output of the LDA function, which was saved in the variables `sections_lda_wephil` and `sections_lda_seaout`, contains beta values for each word showing the degree to which each word matches the given topic in which it is grouped. 
...
```{r, fig.width=12, fig.height=6, fig.align="left"}
top_terms_wephil %>%
...
From this we can visualize, for each book, 10 conceptual topics that are made up of 5 concepts—the 5 words with the highest beta values. By viewing these words in groups, one can understand what ideas are discussed most within each book. This makes the decision of which book to read much easier, and is free of subjective opinions found within book reviews and numeric ranking systems. 
